...
if lsof -i :8080 >/dev/null 2>&1; then
  ...
  read -p "?? No app found on port 8080. Please enter your local Aztec RPC port: " PORT
fi
...
while true; do
  ...
  if [ -z "$REMOTE_RESPONSE" ] || [[ "$REMOTE_RESPONSE" == *"error"* ]]; then
    echo "?? Remote RPC ($REMOTE_RPC) not responding or returned an error."
    REMOTE="N/A"
    send_telegram_message "?? *Remote node error:* No response or error from remote RPC at $REMOTE_RPC (Checked at $timestamp)"
  else
